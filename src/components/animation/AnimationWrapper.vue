    ...
    <div ref="cardContainer" class="relative h-[32rem]">
      <div
        ...
        :class="[
          ...
          showSpread ? intialPositions[index] : spreadPositions[index],
          card.bg,
        ]"
...
const spreadPositions = ["top-0 left-0", "top-0 left-1/3", "top-0 left-2/3"];

onMounted(() => {
  const observer = new IntersectionObserver(
    ([entry]) => {
      // Reverse logic here: collapse on scroll in
      showSpread.value = entry.isIntersecting ? false : true;
    },
    { threshold: 1 }
  );
