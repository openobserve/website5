    ...
    <div class="flex flex-col space-y-4">
      <a v-for="author in authors" :href="generateAuthorLink(type, author.slug)" :key="author.id"
        ...
        <div class="w-full md:w-2/3 p-6 h-44 flex flex-col justify-between cursor-pointer">
          <div>
            <h3 :class="[titleTextColor, 'text-xl font-bold mb-3 underline cursor-pointer']">
              {{ author.name }}
            </h3>
          ...
          <div class="flex space-x-4">
            <a v-if="author.linkedInUrl" :href="author.linkedInUrl" target="_blank" :class="[linkColor, 'underline']">
              LinkedIn
            </a>
            <a v-if="author.twitterUrl" :href="author.twitterUrl" target="_blank" :class="[linkColor, 'underline']">
              Twitter
            </a>
            <a v-if="author.instagramUrl" :href="author.instagramUrl" target="_blank" :class="[linkColor, 'underline']">
              Instagram
            </a>
            <a v-if="author.youtubeUrl" :href="author.youtubeUrl" target="_blank" :class="[linkColor, 'underline']">
              YouTube
            </a>
          </div>
        </div>
