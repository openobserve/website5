    ...
    <div v-for="(item, index) in items" :key="index" class="mb-8 md:mb-0">
      <div :class="[
        ...
        <div class="relative w-full md:w-1/2">
          <!-- Background Image -->
          <div class="absolute inset-0 bg-cover bg-center rounded-lg z-0" :style="{
        ...
        <div class="w-full md:w-1/2 z-10 flex flex-col space-y-2">
          <h2 class="text-[#f4f4f5] font-inter font-semibold text-xl md:text-2xl lg:text-4xl">
            {{ item.title }}
          </h2>
          <div v-if="item.items?.length" class="flex flex-col space-y-2">
            <div v-for="(item, index) in item.items" :key="index" class="flex flex-col space-y-1">
              <h3 class="font-inter font-semibold text-sm md:text-base lg:text-lg text-white">
                {{ item.title }}
              </h3>
