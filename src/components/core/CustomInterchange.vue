...
<template>
  <div :class="['w-full flex flex-col', props.theme]">
    ...
    <div class="w-full flex flex-col lg:flex-row justify-center items-center gap-12 overflow-x-hidden"
      :class="sectionClasses">
      <!-- client review Section -->
      <motion.div :initial="'hidden'" :whileInView="'visible'" :variants="cardVariant"
        :inViewOptions="{ once: true, amount: 0.3 }"
        class="h-auto rounded-lg border border-card bg-white shadow-sm p-6 xl:p-10 z-10 w-full lg:w-1/2">
        <div class="flex flex-col items-start gap-4">
          <div class="flex flex-row items-start gap-3">
            <!-- Icon -->
            <div class="bg-black/5 text-color-primary-blue rounded-md p-4 shrink-0 mt-1">
              <Quote class="w-6 h-6" />
            </div>
      ...
      <!-- Feature List -->
      <motion.div class="w-full lg:w-1/2 z-10 flex flex-col space-y-6" :initial="'hidden'" :whileInView="'visible'"
        :variants="listContainerVariant" :inViewOptions="{ once: true, amount: 0.3 }">
        <div v-if="items?.length" class="flex flex-col space-y-6">
          <motion.div v-for="(item, index) in items" :key="index" class="flex items-start" :variants="listItemVariant">
            <div class="space-y-1">
