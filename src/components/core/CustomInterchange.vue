...
<template>
  <div class="w-full flex flex-col gap-4">
    ...
    <div
      class="w-full flex flex-col lg:flex-row justify-center items-center gap-6 md:gap-12 md:mt-6"
      :class="sectionClasses"
    >
      <!-- Quote Section -->
      <div
        class="rounded-lg border border-gray-200 bg-white shadow-sm p-3 z-10 w-full lg:w-1/2"
      >
      ...
      <div class="w-full lg:w-1/2 z-10 flex flex-col space-y-6">
        <div v-if="items?.length" class="flex flex-col gap-6">
          <div v-for="(item, index) in items" :key="index" class="flex gap-3">
            <Check
              class="h-5 w-5 mt-1 bg-blue-500/20 rounded-full p-1 flex-shrink-0 text-blue-500"
            />
