  ...
  <CustomSection>
    ...
    <div class="text-white py-8">
      ...
      <div v-if="isMobile" class="h-full">
        <swiper :modules="[Pagination]" :space-between="20" :slides-per-view="1" :pagination="{ clickable: true }"
          class="mobile-swiper h-full">
          <swiper-slide v-for="tab in items" :key="tab.id" class="!h-auto">
            <div class="flex flex-col items-center space-y-6 p-4 rounded-lg h-full"
              ...
              <div class="w-full flex justify-center">
                <CustomImage :image="tab.image?.url" cssClass="rounded-lg shadow-lg" />
              </div>
            </div>
          </swiper-slide>
        </swiper>
      ...
      <div v-else class="flex flex-row relative">
        ...
        <div class="w-1/3 lg:w-1/4 border-r border-gray-700 px-2">
          <div class="space-y-4">
            <template v-for="(tab, index) in items" :key="index">
              <button class="flex items-center px-2 py-2 text-left text-lg font-medium border-l-4 transition-all"
                :class="{
        ...
        <!-- Right Content Area -->
        <div class="flex flex-row w-2/3 lg:w-3/4 items-center justify-between rounded-lg mx-10 p-6 space-x-3"
          style="background-color: rgba(53, 59, 64, 1)">
          <!-- Text Content -->
          ...
          <!-- Image -->
          <div class="w-[60%] px-3">
            <CustomImage :image="activeTab.image" cssClass="rounded-lg shadow-sm" />
          </div>
        </div>
      </div>
    </div>
...
.button-color {
  background-color: linear-gradient(rgba(208, 251, 255, 1),
      rgba(119, 139, 255, 1));
}
