...
<template>
  ...
  <div class="relative flex items-center justify-center min-h-[40vh] px-6 sm:px-12 lg:px-24 bg-no-repeat py-8" :style="{
    backgroundImage:
    ...
    <div class="flex flex-col lg:flex-row gap-4 lg:gap-12 items-center w-full container mx-auto">
      <!-- Left Column: Title & Description -->
      <div class="w-full text-left flex flex-col space-y-4">
        <h1 class="text-5xl xl:text-6xl text-white font-bold">{{ title }}</h1>
        <Heading :description="description" align="LEFT"
          class="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-300" />
      </div>

      <!-- Right Column: Form Placeholder -->
      <div class="w-full flex justify-center">
        <div class="bg-black bg-opacity-40 rounded-2xl p-8 shadow-lg backdrop-blur-md w-full">
          <form @submit.prevent="onSubmit">
            <div class="space-y-3">
              <div>
                ...
                <span v-if="name.errorMessage.value" class="text-xs text-red-500">{{ name.errorMessage.value }}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <CustomInput v-model="phone.value.value" name="phone" type="phone" placeholder="Enter here" />
              ...
              <div>
                ...
                <span v-if="message.errorMessage.value" class="text-xs text-red-500">{{ message.errorMessage.value
                }}</span>
              </div>
              <!-- Terms & Conditions -->
              <div class="flex flex-col items-start gap-1">
                <div class="flex items-center gap-2">
                  <!-- Checkbox -->
                  <input id="terms" type="checkbox" v-model="terms.value.value" class="h-4 w-4 cursor-pointer"
                  ...
                  <label for="terms"
                    ...
                    <a href="/policies/terms-of-service" class="text-blue-500 underline text-sm whitespace-nowrap"
                      target="_blank">
                ...
                <p v-if="terms.errorMessage.value" class="text-xs text-red-500 min-h-[1rem]">
              ...
              <!-- Submit Button -->
              <CustomButton variant="secondary" type="submit" :disabled="loading" class="mt-4">
