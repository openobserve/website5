...
<template>
  <CustomSection>
    <div class="grid-container md:px-4 md:-mt-16 md:z-10">
      <div v-for="(item, index) in items" :key="index" :id="slugify(item.title)"
        class="grid-item rounded-sm bg-[#131219] w-full h-full">
        <!-- Content Layer -->
        <div class="flex flex-col p-6 h-full gap-4">
          <!-- Icon and Title Row -->
          <!-- Icon/Logo -->
          <CustomImage v-if="item?.image" :image="item?.image" class="w-10 h-10" />
          <!-- Title -->
...
.grid-item:nth-last-child(2):nth-child(3n + 1) {
  grid-column: 2 / 4;
  /* First of two last items */
}

.grid-item:nth-last-child(1):nth-child(3n + 2) {
  grid-column: 4 / 6;
  /* Second of two last items */
}
